...
pipelines (20 for Java and the baseline generators, 30 for the LLM Python suites), an
inconsistency inherited from separate implementations. It binds exactly one function in the
whole corpus (CP-025, 24 candidate sites) and cannot affect T4, since that tier asks only
whether at least one mutant is killed and CP-025 kills 18; it bounds only the resolution of
the continuous score for that one function.
...
above was corrected to match the registered definition of T2. Eight functions under the
baseline prompt and ten under the target-specified prompt were affected, against two for each
of the two Java baselines --- so the correction helps the model more than it helps its
