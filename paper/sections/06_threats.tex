% NGUOI VIET: RW (Loc). 2026-07-17 (v2): ban TAY moi cua Loc,
% DA VERIFY human — ZeroGPT 0% + GPTZero 81% human ("entirely human").
% Wire boi RW/Claude + fix fact (giu nguyen giong da scan):
%  (1) FACT: parenthetical "(it was separateClassLoader=false)" DAO NGUOC — nguyen nhan la
%      separateClassLoader=TRUE (bat), fix = DISABLE no; sua lai cho dung notes.md;
%  (2) "commons-math4" -> "commons-math" (dong bo ten dataset); (3) casing java->Java v.v.;
%  (4) placeholder (Section Method/Results) -> \ref; (5) \texttt cho identifier.
% Fact 3 o mutation thieu (JA-017/048/058) + JDK 11/8 hon hop: DA verify audit 17/07.
...
Python dataset stayed at 0.0\% (Section~\ref{sec:results}), which we read as a genuine, if
weak, finding about the tool's real-world difficulty on these modules rather than a
leftover environment defect.
