...
public class SetOperationCardinalityHelperTest {
    ...
    void testFilter_NullCollection() {
        assertFalse(SetOperationCardinalityHelper.filter(null, x -> true));
    }
    ...
    void testFilter_EmptyCollection() {
        ...
        assertFalse(SetOperationCardinalityHelper.filter(collection, x -> x < 0));
    ...
    void testFilter_SomeElementsRemoved() {
        ...
        assertTrue(SetOperationCardinalityHelper.filter(collection, x -> x % 2 == 0));
    ...
    void testFilter_AllElementsRemoved() {
        List<Integer> collection = new ArrayList<>(Arrays.asList(1, 2, 3));
        assertTrue(SetOperationCardinalityHelper.filter(collection, x -> x > 3));
        assertTrue(collection.isEmpty());
    }
}