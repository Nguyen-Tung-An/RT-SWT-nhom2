...
def test_iter_content_decode_error():
    response = Response()
    response.raw = type('Raw', (), {'stream': lambda chunk_size, decode_content: (_ for _ in ()).throw(DecodeError)})()
    with pytest.raises(ContentDecodingError):
        list(response.iter_content(chunk_size=5))

def test_iter_content_read_timeout_error():
    response = Response()
    response.raw = type('Raw', (), {'read': lambda chunk_size: (_ for _ in ()).throw(ReadTimeoutError)})()
