...
def test_run_command_success(mock_script_info):
    run_command(mock_script_info, '127.0.0.1', 5000, False, False, False, None, None, None)

def test_run_command_with_reload(mock_script_info):
    run_command(mock_script_info, '127.0.0.1', 5000, True, False, False, None, None, None)

def test_run_command_with_debugger(mock_script_info):
    run_command(mock_script_info, '127.0.0.1', 5000, False, True, False, None, None, None)

def test_run_command_with_ssl(mock_script_info):
    cert = ('cert.pem', 'key.pem')
    run_command(mock_script_info, '127.0.0.1', 5000, False, False, False, cert, None, None)
