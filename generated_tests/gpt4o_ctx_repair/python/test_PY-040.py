...
class MockParameter(click.Parameter):
    def _parse_decls(self, ctx, param_decls):
        return param_decls
...
def test_validate_key_adhoc_cert():
    ...
    with pytest.raises(click.BadParameter, match='When "--cert" is "adhoc", "--key" is not used.'):
        _validate_key(ctx, MockParameter(name="key"), "some_key")
...
def test_validate_key_required_when_cert_provided():
    ctx = click.Context(click.Command("test"))
    ctx.params = {"cert": "some_cert"}
    with pytest.raises(click.BadParameter, match='Required when using "--cert".'):
        _validate_key(ctx, MockParameter(name="key"), None)

def test_validate_key_success():
    ctx = click.Context(click.Command("test"))
    ctx.params = {"cert": "some_cert"}
    result = _validate_key(ctx, MockParameter(name="key"), "some_key")
