...
def test_load_app_with_app_import_path():
    ...
    with mock.patch('flask.cli.prepare_import', return_value='myapp'):
        with mock.patch('flask.cli.locate_app', return_value=Flask(__name__)):
            app = script_info.load_app()
            assert app is not None
            assert isinstance(app, Flask)
...
def test_load_app_with_no_app_found():
    ...
    with mock.patch('flask.cli.locate_app', return_value=None):
