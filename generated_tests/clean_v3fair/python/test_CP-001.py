...
def test_merge_hooks_both_non_empty():
    ...
    assert result == merge_setting(request_hooks, session_hooks, OrderedDict)

def test_merge_hooks_with_different_types():
    request_hooks = {'response': [lambda r, *args, **kwargs: r]}
    session_hooks = {'response': [lambda r, *args, **kwargs: r], 'other': [lambda r: r]}
    result = merge_hooks(request_hooks, session_hooks)
    assert 'other' in result
    assert result['other'] == session_hooks['other']