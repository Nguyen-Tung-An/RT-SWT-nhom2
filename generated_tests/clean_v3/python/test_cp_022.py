...
class TestInjectUrlDefaults:
    ...
    def test_inject_url_defaults_with_valid_endpoint(self, app):
        app.url_default_functions['test'] = [lambda endpoint, values: values.update({'default_key': 'default_value'})]
    ...
    def test_inject_url_defaults_with_empty_values(self, app):
        app.url_default_functions['test'] = [lambda endpoint, values: values.update({'key': 'value'})]
        with app.test_request_context('/'):
            values = {}
            app.inject_url_defaults('test.endpoint', values)
    ...
    def test_inject_url_defaults_with_multiple_functions(self, app):
        app.url_default_functions['test'] = [
    ...
    def test_inject_url_defaults_with_blueprint(self, app):
        app.url_default_functions['blueprint'] = [lambda endpoint, values: values.update({'bp_key': 'bp_value'})]
        with app.test_request_context('/'):
            values = {}
            app.inject_url_defaults('blueprint.endpoint', values)
            assert values == {'bp_key': 'bp_value'}

    def test_inject_url_defaults_with_invalid_endpoint_format(self, app):
        with app.test_request_context('/'):
            values = {}
            app.inject_url_defaults('invalidendpoint', values)
            assert values == {}