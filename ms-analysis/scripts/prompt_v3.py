...
def target_spec(rec: dict) -> str:
    ...
    if rec["language"] == "java":
        ...
        if rec["is_method"] == "yes":
            ...
            # v3: khong chi noi "construct the class first" ma CHI RO cach dung.
            # Doan duoi da duoc THUC THI that trong enrich_constructors.py, khong suy dien.
            ...
            elif rec.get("ctor_note"):
                lines.append("NOTE on construction   : direct construction fails "
                             f"({rec['ctor_note'][:60]}); build the receiver via the "
                             "public API of the package instead.")
            if rec.get("needs_context") == "yes":
                lines.append(
                    "CONTEXT: Flask objects often need an active application/request "
                    "context. If the call raises 'Working outside of ...context' or "
                    "'There is no request in this context', wrap it:\n"
...
def build_prompt(rec: dict) -> str:
    ...
    return (
        ...
        "4. Do NOT invent helper classes or mocks for the receiver. Build the real object "
        "using the constructor shown above.\n"
        "5. Only import names you are certain exist. If unsure which module a helper class "
        "lives in, trigger the condition through the public API instead of importing it.\n"
